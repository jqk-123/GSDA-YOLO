...
'''
New GhostConv module added
GhostConv uses two convolutional layers to generate more feature maps and improves computational efficiency by reducing the number of parameters.
cv1 is a standard convolution used to generate the basic feature map.
cv2 is a depthwise separable convolution used to generate more feature maps.
Modify the GSConv module to GSConvWithGhostConv:
Replace the original Conv module with GhostConv to reduce the number of parameters and improve computational efficiency.
The feature maps generated by GhostConv are concatenated and the channels are rearranged, maintaining the original structure and function of GSConv.
By introducing GhostConv, the GSConvWithGhostConv module maintains a high feature extraction capability while
It reduces the number of parameters and computational load, making it suitable for lightweight real-time object detection tasks.
